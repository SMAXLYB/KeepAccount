...
import life.chenshi.keepaccounts.module.record.databinding.RecordActivityNewRecordBinding
...
class NewRecordActivity : BaseActivity() {
    ...
    private val mBinding by bindingContentView<RecordActivityNewRecordBinding>(R.layout.record_activity_new_record)
    ...
    override fun initListener() {
        ...
        mCommonCategoryAdapter.setOnItemClickListener { _, category, _ ->
            ...
            if (category.id == -1) {
                ...
                // Navigation.findNavController(mBinding.root).navigate(request)
            } else {
        ...
        mNewRecordViewModel.run {
            ...
            currentRecordType.observe(this@NewRecordActivity) {
                mBinding.tvRecordType.text = if (it == RECORD_TYPE_INCOME) {
                    mBinding.tvMoney.setTextColor(getColorById(R.color.common_income))
                    "收入"
                } else {
                    mBinding.tvMoney.setTextColor(getColorById(R.color.common_outcome))
                    "支出"
                }
